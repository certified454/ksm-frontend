...
export default function ChallengeDetail() {
    ...
    const [pools, setPools] = useState<{ optionText: string }[]>([]);
    ...
    const fetchVotes = async (pageNum = 1, refresh = false) => {
            try {
                ...
                const response = await fetch(`${API_URL}/votes?page=${pageNum}&limit=20`,
                    {
                        method: "GET",
                        headers: {
                            Authorization: `Bearer ${token}`,
                ...
                const uniqueVote = refresh || pageNum === 1
                    ? data.challenges
                    : Array.from(new Set([...votes, ...data.challenges].map((vote) => vote._id))).map((id) => [
                        ...votes, ...data.challenges].find((vote) => vote._id === id)
                )
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            if (challenge?.startDate && challenge?.time) {
                // Combine startDate and time into a single Date object
                const start = new Date(challenge.startDate);
                const time = new Date(challenge.time);
    
                // Set hours and minutes from time to start
        ...
        <>
            <ScrollView contentContainerStyle={{ backgroundColor: '#fff'}}>
                <View style={styles.container}>
                    {isLoading ? (
                            <ActivityIndicator size="large" color="#4B0082" style={{top: 350}}/>
                        ) :user?.isOwner ? (
                                ...
                                <View style={styles.challengeItemsContainer}>
                                    { challenge ?  (
                                        ...
                                        <View style={styles.challengeItem}>
                                            <TouchableOpacity onPress={() => setModalVisible(true)} style={styles.currentTime}>
                                                <Ionicons name="chatbox-outline" size={24} color="#4B0082" />
                                                <Text style={styles.currentTimeText}>
                                                    {formatVoteCount(challenge.voteCount)} Entries
                                                </Text>
                                            </TouchableOpacity>
                                        ...
                                        <Text>Challenge not available</Text>
                                    )}
                                </View>
                            </View>
                        ) : challenge ?  (
            ...
            <Vote isVisible={modalVisible} onClose={() => setModalVisible(false)}>
                <ScrollView style={styles.challengeVotes}>
                    <View style={styles.votes}>
                        <TouchableOpacity onPress={() => handleprofilePicturePress(user._id)} style={styles.vote}>
                            <Image source={{ uri: user.profilePicture }} style={styles.profilePicture} />
                            <Text style={styles.username}>{user.username}</Text>
                        </TouchableOpacity>
                    <Text style={styles.totalVotes}>{item?.text}</Text>
                    </View>
