...
function parseCaption(
  ...
  const normalizedTags = tags.map(t => typeof t === 'string' ? { name: t } : t);
...
export default function Index() {
  ...
  //search variable
  const [searchResults, setSearchResults] = useState<{users: any[]; tags: any[]}>({users: [], tags: []});
  ...
  const pickHomeTeamLogo = async () => {
    try {
      if (Platform.OS !== "web") {
        ...
        if (status !== "granted") {
          Alert.alert('Permission Denied', 'You need to allow access to the media library to pick an image');
          return;
        }
      }
      ...
      if (!result.canceled && result.assets && result.assets.length > 0) {
        ...
        setHomeTeamBase64(selectedAsset.base64 ? `data:image/*;base64,${selectedAsset.base64}` : selectedAsset.uri);
      } else {
        console.log('failed')
      }
    } catch (error) {
  ...
  const pickAwayTeamLogo = async () => {
    try {
      ...
      if (!result.canceled && result.assets && result.assets.length > 0) {
        ...
        setAwayTeamBase64(selectedAsset.base64 ? `data:image/*;base64,${selectedAsset.base64}` : selectedAsset.uri);
      } else {
        console.log('failed')
      }
    } catch (error) {
  ...
  const createMatch = async () => {
    ...
    try {
      ...
      const homeType = homeExtention ? `image/${homeExtention.toLowerCase()}` : "image/jpg";
      ...
      const awayType = awayExtention ? `image/${awayExtention.toLowerCase()}` : "image/jpg";
  ...
  const fetchMatches = async (pageNum = 1, refresh = false) => {
    setLoading(true);
    try {
      const response = await fetch(`${API_URL}/match?page=${pageNum}&limit=10`, {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${token}`
        }
      ...
      const uniqueMatches = (refresh || pageNum === 1
        ? data.matches
        : Array.from(new Set([...matches, ...data.matches].map((match) => match._id))).map((id) =>
            ({ ...( [...matches, ...data.matches].find((match) => match._id === id) ) })
          )
      );
      ...
      <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
        <View style={styles.container}>
          <View style={styles.header}>
            ...
            {isPostOwner ? (
              <View>
                <TouchableOpacity onPress={() => showOptions(item._id)}>
                ...
                {optionsDropdownId === item._id && (
                  <View style={styles.optionsDropdown}>
                      ...
                      <View style={styles.editPost}>
                        <TouchableOpacity onPress={() => { setOptionsDropdownId(null); modalClicked(item._id) }} style={styles.post}>
                          <View style={styles.select}></View>
                          <Text style={styles.text4}>Edit Post</Text>
                        </TouchableOpacity>
                        <TouchableOpacity onPress={() => {setOptionsDropdownId(null);  handleDeletePost(item._id);}} style={styles.post}>
                          <View style={styles.select}></View>
                      ...
                      <TouchableOpacity onPress={() => setOptionsDropdownId(null)} style={styles.backPost}>
              ...
              <View>
                ...
                {optionsDropdownId === item._id && (
                  <View style={styles.optionDropdown}>
                      ...
                      <View style={styles.editPost}>
                        <TouchableOpacity onPress={() => { setOptionsDropdownId(null); handleRequestChallenge(item.user._id); }} style={styles.post}>
                          <View style={styles.select}></View>
                ...
                <TouchableOpacity style={styles.commentIcons} onPress={() => handlePostPress(item._id)}>
                  <View style={styles.likesSection}> 
                    <View style={styles.likesCounts}>
                      ...
                      <Text style={styles.textcomment}> {formatComments(item.likesCount)}</Text>
                    </View>    
                  </View>
                </TouchableOpacity>
                <TouchableOpacity style={styles.commentIcons}onPress={() => handlePostPress(item._id)}>
  ...
  const renderCombinedSearch = ({ item }: { item: any }) => {
      ...
      if (item.type === 'tag') {
        return (
          <TouchableOpacity onPress={() => handleTagPress(item.name)} style={styles.userContainer}>
            {item.image && (
              <Image style={styles.image} contentFit='cover' source={{ uri: item.image }} />
            )}
        ...
        <View  style={styles.containerItem}>  
          { user && user.isOwner ? (
            <>
                  ...
                  <TextInput 
                    ...
                    <>
                      <FlatList
                        ...
                        contentContainerStyle={styles.container}
                        ListEmptyComponent={
                          <Text style={{ textAlign: 'center', marginTop: 20 }}>No user or tag found</Text>
                        }
                      />
                            ...
                            <View style={styles.captionContainer1}>
                              ...
                              <View style={styles.updatePostButtonContainer}>
                                  {isUpdatingPost ?(
                                    ...
                                    <TouchableOpacity style={styles.updatePostButton} onPress={() => handleEditPost(editingPostId)}>
              ...
              <Match isVisible={modalVisible} onClose={() => setModalVisible(false)}>
                  <View style={styles.matchContainer}>
                    ...
                    <View>
                      <TouchableOpacity onPress={pickHomeTeamLogo} >
                        {homeTeamBase64 ? (
                          <View>
                            <Image source={{ uri: homeTeamBase64 }} style={{ width: 100, height: 100, borderRadius: 50, left:"15%" }} />
                          </View>
                        ): (
                          <View>
                    ...
                    <View>
                      <TouchableOpacity onPress={pickAwayTeamLogo} >
                        {awayTeamBase64 ? (
                          <View>
                            <Image source={{ uri: awayTeamBase64 }} style={{ width: 100, height: 100, borderRadius: 50, left:"15%" }} />
                          </View>
                        ): (
                          <View>
                            <Ionicons  name="camera" size={30} color={"#4B0082"} />
                    ...
                    <TouchableOpacity onPress={createMatch} style={styles.updatePostButtonContainer}>
                      {submiting ? (
                        <ActivityIndicator size="small" color="#fff" />
                      ) : (
