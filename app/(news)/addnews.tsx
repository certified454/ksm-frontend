...
import styles from "../../assets/styles/addnews";
...
export default function addNewsLayout() {
    ...
    const selectPicture1 = async () => {
        ...
        if (!result.canceled && result.assets && result.assets.length > 0) {
            ...
            setPicture1Base64(selectedAsset.base64 ? `data:image/*;base64,${selectedAsset.base64}` : selectedAsset.uri);
        }  
    }
    ...
    const selectPicture2 = async () => {
        ...
        if (!result.canceled && result.assets && result.assets.length > 0) {
            ...
            setPicture2Base64(selectedAsset.base64 ? `data:image/*;base64,${selectedAsset.base64}` : selectedAsset.uri);
        }  
    }
    ...
    const handleAddNews = async () => {
        try {
            ...
            const picture1Type = picture1Exten ? `image/${picture1Exten.toLowerCase()}` : 'image/jpg';
            ...
            const picture2Type = picture2Exten ? `image/${picture2Exten.toLowerCase()}` : 'image/jpg';
    ...
    return (
        <SafeAreaProvider>
            <SafeAreaView style={{flex: 1}}>
                {isLoading ? (
                    <ActivityIndicator size="large" color="#4B0082" style={{marginTop: 120}}/>
                ): (
                    <View style= {styles.container}>
                        <View style={styles.header}>
                            <View style={styles.backIcon}>
                                <MaterialIcons name="arrow-back-ios" size={25} color="#4B0082" onPress={() => router.back()}/>
                            </View>
                            <Text style={styles.title}>Add News</Text>
                        </View>

                        <View style={styles.deiscriptionContainer}>
                            <Text>* Enter Description</Text>
                            <TextInput
                                style={ styles.deiscriptionTextInput }
                                value={description}
                        ...
                        <View style={styles.imagesContainer}>
                            <View style={styles.picture1Container}>
                                <Text style={styles.text}>* Select First Image to be display</Text>
                                <TouchableOpacity onPress={selectPicture1} style={styles.picture1}>
                                    {picture1Base64 ? ( 
                                        <Image source={{ uri: picture1Base64 }} style={styles.picture1} />
                                    ): (
                            ...
                            <View style={styles.picture1Container}>
                                <Text style={styles.text}>* Select Second Image to be display</Text>
                                <TouchableOpacity onPress={selectPicture2} style={styles.picture2}>
                                    {picture2Base64 ? (
                                        <Image source={{ uri: picture2Base64 }} style={styles.picture2} />
                                    ): (
                       ...
                       <TouchableOpacity onPress={handleAddNews} style={styles.submitButton} disabled={submiting}>
                            { submiting ? (
