...
//format date range like "Sep 10 - Sep 15"
...
// converrt date to a more readable format: like Jan 01
export function calculateTimeRemaining(startDate, endDate) {
  const now = new Date();
  const start = new Date(startDate);
  const end = new Date(endDate);

  // If event hasn't started yet, show time until start
  // If event is active, show time until end
  // If event already ended, return "ended"
  let diffInMs;
  if (now < start) {
    diffInMs = start - now;
  } else if (now <= end) {
